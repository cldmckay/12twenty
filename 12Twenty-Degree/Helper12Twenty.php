		...
		function PutStudent(string $student, string $id, string $studentID){
			...
			try{
				...
				if(isset($response["RoleId"])){					
					...
					var_dump($student);
					return "Error with student data! Method: PATCH, ID:".$studentID;					
				}
			}catch (Exception $e) {
				return 'Caught exception PATCH: '.$e->getMessage()." with student".$student->Id;
			}
		}
		
		function PatchStudent(string $student, string $id, string $studentID){

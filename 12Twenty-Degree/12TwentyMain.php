<?php
	...
	ini_set('include_path', '/groups/iuieapi/bin/12Twenty-Degree/:'.get_include_path());
	...
	Class MainApp{
		...
		public function __construct(){
			...
			$this->helperDenodo = new HelperDenodo($test, $debug);
			
			$this->logProgress("Script starts");
			
			$filePath12Twenty = "/groups/iuieapi/bin/12TwentyDegreeFiles/";
			...
			$filePathDenodo = "/groups/iuieapi/bin/DenodoDegreeFiles/";	
			...
			$this->logProgress("Combining Student records with same degree level...");
			...
			for($i=0; $i < count($student_list); $i++){
				for($j=0; $j< count($student_list[$i]->collegeArr); $j++){
					$student_list[$i]->collegeArr[$j]->setID($college_array);
				}
				for($j=0; $j< count($student_list[$i]->department); $j++){
					$student_list[$i]->department[$j]->setID($department_array);
				}
				for($j=0; $j< count($student_list[$i]->majorArr); $j++){
					$student_list[$i]->majorArr[$j]->setID($under_major_array);
				}
				for($j=0; $j< count($student_list[$i]->degree); $j++){
					$student_list[$i]->degree[$j]->setID($degree_array);
				}
			...
			shell_exec('rm -r /groups/iuieapi/bin/DenodoDegreeFiles/*');
			$this->SendLog();
		}
				...
				for($j=0; $j < count($studentList_new); $j++){
					...
					if($studentList_new[$j]->StudentId == $student_list[$i]->StudentId){
						if($studentList_new[$j]->DegreeLevel == $student_list[$i]->DegreeLevel){							
							...
							$studentList_new[$j]->AddCertificateAndMinor($student_list[$i]);

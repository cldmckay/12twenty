<?PHP
	class HelperDenodo{
		...
		public function __construct($test, $debug) {
		...
		function SaveStudentDataToFile(){
			$studentArray = array();
			$remote_url = 'https://ebidvt.uits.iu.edu/server/iu_webtech_iuieapi/iuieapi_degree/views/iuieapi_degree?$format=XML';
			
			if($this->test){
				$remote_url = 'https://ebidvt-dev.uits.iu.edu/server/iu_webtech_iuieapi/iuieapi_degree/views/iuieapi_degree?$format=XML';
			}			
			...
			try{
				...
				if(strlen($file) > 100){
					file_put_contents("/groups/iuieapi/bin/DenodoDegreeFiles/students.txt", $file);	
					$file = file_get_contents("/groups/iuieapi/bin/DenodoDegreeFiles/students.txt", false);	
					$xml = simplexml_load_string($file);					
					$student_count = 0;
					foreach($xml->{'iuieapi_degree'} as $student){
				...
				foreach($xml->{'iuieapi_degree'} as $student){
					/*if ($this->test) {
						if ($student_count > 5000) {
							echo "Breaking out of loop at student count: $student_count\n";
							break;
						}
					}  */
					$json = json_encode($student);
